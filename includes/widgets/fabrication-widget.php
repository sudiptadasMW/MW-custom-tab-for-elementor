...
class Fabrication_Widget extends Widget_Base
{
	...
	protected function register_controls()
	{
		...
		$this->add_control('nav_active_bg_color', [
			'label' => esc_html__('Active Nav Background', 'mw-custom-tab'),
			'type' => Controls_Manager::COLOR,
			'selectors' => [
				'{{WRAPPER}} .mw-tab-nav-item.active' => 'background-color: {{VALUE}};',
			],
		]);

		$this->add_group_control(Group_Control_Typography::get_type(), [
			'name' => 'active_nav_typography',
			'label' => esc_html__('Typography', 'mw-custom-tab'),
			'selector' => '{{WRAPPER}} .mw-tab-nav-item.active, {{WRAPPER}} .mw-sub-tab-item.active',
		]);
	...
	private function render_style_one(array $settings)
	{
		...
		<div class="mw-style-one-wrapper">
			<!-- Left Nav -->
			<nav class="mw-tab-nav" role="tablist" aria-orientation="vertical"
				aria-label="<?php esc_attr_e('Service tabs', 'mw-custom-tab'); ?>">
				<?php foreach ($tabs as $index => $tab):
					$is_active = (0 === $index);
					$tab_id = 'mw-s1-tab-' . $widget_id . '-' . $index;
					?>
					<div class="mw-tab-nav-item<?php echo $is_active ? ' active' : ''; ?>" role="tab"
						id="<?php echo esc_attr($tab_id); ?>" aria-selected="<?php echo $is_active ? 'true' : 'false'; ?>"
						aria-controls="<?php echo esc_attr($tab_id . '-panel'); ?>"
						tabindex="<?php echo $is_active ? '0' : '-1'; ?>" data-index="<?php echo esc_attr($index); ?>">
			...
			<div class="mw-tab-panels">
				<?php foreach ($tabs as $index => $tab):
					...
					<div class="mw-tab-panel<?php echo $is_active ? ' active' : ''; ?>"
						id="<?php echo esc_attr($tab_id . '-panel'); ?>" role="tabpanel"
						aria-labelledby="<?php echo esc_attr($tab_id); ?>" data-index="<?php echo esc_attr($index); ?>">
						<?php if ($img_url): ?>
	...
	private function render_style_two(array $settings)
	{
		...
		<div class="mw-style-two-wrapper">
			...
			<div class="mw-top-tabs" role="tablist" aria-orientation="horizontal"
